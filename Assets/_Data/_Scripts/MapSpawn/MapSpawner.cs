...
{
    //Public
    public float initScrollSpeed = 10f;
    ...
    {
        currentMap = ObjectPooler.Instance.SpawnFromPool("Map", 0, StartSpawnMapPoint, Quaternion.identity);
    ...
    private GameObject SpawnRandomMap(Vector3 position)
    {
        ...
        AssignIndexMapSpawned(newIndex);

        return obj;
    }

    //Random map sẽ spawn sao cho không trùng lặp với map hiện tại
    private int RandomIndex()
    {
        ...
        while (index == indexMapSpawned);

        return index;
    }

    //Gán index vừa spawn vào indexMapSpawned để không bị trùng lặp cho lần spawn tiếp theo
    private void AssignIndexMapSpawned(int newIndex) => indexMapSpawned = newIndex;
    //Di chuyển Map
