...
public class StartGameCutScene : Singleton<StartGameCutScene>
{
    ...
    public void StartCutScene()
    {
        ...
        sequence.OnComplete(() =>
        {
            MapSpawner.Instance.SetScrollSpeed(MapSpawner.Instance.initScrollSpeed);
    ...
    public void StartCutScenePVP()
    {
        ...
        sequence.AppendCallback(() =>
        {
            foreach (var player in playerGameObject) //Bật tất cả player
            {
            ...
            foreach (var player in playerRigidbody) //Đặt gravityScale của tất cả player về 0
            {
        ...
        for (int i = 0; i < playerGameObject.Count; i++)
        {
            ...
            sequence.Append(playerPosition[i].DOMove(startingPoint.position + playersOffset[i], 1.5f)
                                                        .SetEase(Ease.OutBack));
        }
        ...
        {
            MapSpawner.Instance.SetScrollSpeed(MapSpawner.Instance.initScrollSpeed);
            foreach (var player in playerRigidbody) //Đặt gravityScale của tất cả player về 7
            {
    ...
    private IEnumerator TriggerGameModeEventNextFrame() //Gọi sự kiện GameMode: Normal/PVP
    {
